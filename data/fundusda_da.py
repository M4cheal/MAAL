...
class FundusDA(Dataset):
    ...
    def batch_generate(self, image_dir, mask_dir):
        ...
        if mask_dir is not None:
            for fname in rgb_filename_list:
                cls_filepath_list.append(os.path.join(mask_dir, fname[:-4]+'.bmp'))
    ...
    def __getitem__(self, idx):
        ...
        if len(self.cls_filepath_list) > 0:
            mask = imread(self.cls_filepath_list[idx]).astype(np.long) #TODO: -1
            if self.transforms_image is not None:
                image = self.transforms_image(image=image)['image']
...
class FundusDALoader(DataLoader, ConfigurableMixin):
    def __init__(self, config, is_eval=False):
        ConfigurableMixin.__init__(self, config)
        if is_eval:
            dataset = FundusDA(self.config.image_dir, self.config.mask_dir, self.config.transforms)
        else:
            dataset = FundusDA(self.config.image_dir, self.config.mask_dir, self.config.transforms, self.config.transforms_image)
        ...
        super(FundusDALoader, self).__init__(dataset,
